...
# S: ゆうパケット系（薄物）
# M: ゆうパケットプラス
# L: ゆうパック60-80想定（安全側の目安）
# XL: ゆうパック100-120想定（安全側の目安）
SHIPPING_TABLE_YEN = {
    "S": 230,
    "M": 455,
    "L": 770,
        ...
        try:
            ...
            if msg_type == "image":
                ...
                msg = (
                    ...
                    f"送料を正確にする場合（任意）：\n"
                    f"・サイズ S/M/L/XL（S=薄物, M=ゆうパケットプラス, L=ゆうパック60-80, XL=ゆうパック100-120）\n"
                    f"・重さ 850g または 1.2kg（重い物は入力推奨）\n\n"
                    f"次に価格を送ってください：\n"
                    f"・仕入れ 980\n"
                    f"・売れた 2800（実相場）\n"
                    f"・売値 2800（希望売値）\n\n"
